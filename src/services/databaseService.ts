...
// Initialize mock storage at the start
if (typeof window !== 'undefined') {
  window.quizAttempts = window.quizAttempts || [];
  window.mockQuizzes = window.mockQuizzes || [];
}

// Function to safely handle the case when profile name is not found
export const getAttemptWithProfile = async (attemptId: string) => {
  try {
    ...
    // Then fetch the profile separately using the student_id
    ...
    if (profileError) {
      ...
      // Continue without profile name rather than failing completely
    }
    
    // Return the attempt with the profile name (or 'Unknown User' if not found)
    return {
      ...attempt,
...
export const getQuizzes = async () => {
  try {
    ...
    // Always return mock quizzes for development
    const mockQuizzes = [
      { 
        id: 'mock-1', 
    ...
    // Check for user-created quizzes stored in window.mockQuizzes
    const userCreatedQuizzes = typeof window !== 'undefined' && window.mockQuizzes ? window.mockQuizzes : [];
    
    // Merge real data with mock data, ensuring no duplicates
    ...
    // Return mock quizzes and user-created quizzes as fallback
    const userCreatedQuizzes = typeof window !== 'undefined' && window.mockQuizzes ? window.mockQuizzes : [];
    return [
      { 
        id: 'mock-1', 
...
export const getAttemptsByStudentId = async (studentId: string) => {
  try {
    ...
    // If no attempts found in the database, check the window object
    if (!data || data.length === 0) {
    ...
    // Try to return local attempts as a fallback
    if (typeof window !== 'undefined' && window.quizAttempts) {
      const localAttempts = window.quizAttempts.filter(a => a.studentId === studentId);
      if (localAttempts.length > 0) {
        return localAttempts;
...
export const getStudentRankings = async () => {
  try {
    // Return consistent Indian student rankings with varied scores
    return [
...
export const generateQuestionFeedback = async (questionData: any) => {
  try {
    // For development, mock the feedback generation
    ...
    // Simple mock feedback based on whether the answer is correct
    if (correctAnswer) {
      if (isCorrect) {
...
export const createQuiz = async (quizData) => {
  ...
  try {
    // For development purposes, simulate successful quiz creation
    ...
    // Store the new quiz in window so it can be retrieved later
    if (typeof window !== 'undefined') {
      window.mockQuizzes = window.mockQuizzes || [];
...
// Mock function to provide learning insights
export const getMockLearningInsights = (attemptCount = 0) => {
  // If no attempts, show default insights
  if (attemptCount === 0) {
    return {
...
// Get quiz by ID (useful for taking quizzes)
...
export const submitQuizAttempt = async (attemptData) => {
  ...
  try {
    ...
    // Store this attempt globally so it can be retrieved
    if (typeof window !== 'undefined') {
      window.quizAttempts = window.quizAttempts || [];
...
export const getAttemptById = async (attemptId) => {
  try {
    ...
    // Fallback to mock data if not found in global storage
    console.log(`Fetching attempt with ID: ${attemptId}`);
    
    // Create a mock attempt if not found
    const mockAttempt = {
      id: attemptId,
      quizId: 'mock-1',
      studentId: 'student-1',
    ...
    // Add it to window.quizAttempts so it can be retrieved later
    if (typeof window !== 'undefined') {
      window.quizAttempts = window.quizAttempts || [];
