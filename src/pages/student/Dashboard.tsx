...
const StudentDashboard = () => {
  ...
  // Add state for insights to fix undefined variables error
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Get all quizzes for upcoming quizzes section
        ...
        if (user?.id) {
          ...
          // Fetch insights if there are any attempts
  ...
  // Function to handle a completed quiz
  const refreshAfterQuizCompletion = () => {
    ...
    if (user?.id) {
      getAttemptsByStudentId(user.id).then(attemptsData => {
        ...
        // Trigger a refresh of the insights component
        setInsightsRefreshCounter(prev => prev + 1);
      });
    }
  };

  // Simulate quiz completion for demo purposes (remove in production)
  useEffect(() => {
    ...
    const simulateQuizCompletion = () => {
      ...
      if (urlParams.get('completed') === 'true') {
        ...
        // Remove the parameter to prevent constant refreshing
  ...
  // Calculate improvement rate based on most recent vs earlier attempts
  let improvement = 0;
  ...
  if (recentAttempts.length >= 2) {
    ...
    // Get most recent and the one before
  ...
  // Extract variables from learningInsights to use in the UI
