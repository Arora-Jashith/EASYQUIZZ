...
const TakeQuiz = () => {
  ...
  useEffect(() => {
    const fetchQuiz = async () => {
      ...
      try {
        ...
        if (quizData) {
          // Transform the data to match Quiz type
          ...
          // Only access questions property if it exists in quizData
          if ('questions' in quizData && Array.isArray(quizData.questions)) {
            // Add the quizId property to each question and ensure type is a valid QuestionType
            transformedQuiz.questions = quizData.questions.map(q => {
              // Validate the question type is a valid QuestionType
              const validType: QuestionType = validateQuestionType(q.type);
              
              return {
                ...q,
                quizId: quizData.id,
                type: validType // Ensure type is a valid QuestionType
              };
            });
          }
          ...
          // Initialize answers object only if questions exist
  ...
  const validateQuestionType = (type: string): QuestionType => {
    ...
    // Default to 'essay' if type is not valid
  ...
  const calculateScore = (quiz: Quiz, userAnswers: Record<string, string>): { score: number, answerResults: Answer[] } => {
    ...
    for (const question of quiz.questions) {
      ...
      if (question.type === 'multiple-choice' && question.correctAnswer) {
        ...
        // For essay and other non-multiple choice questions
        answerResults.push({
          questionId: question.id,
          answer: userAnswer
        });
    ...
    // For multiple choice questions, calculate percentage score
    ...
    const score = mcQuestions.length > 0
      ...
      : 0; // If no MC questions, score is 0 (will be updated later with AI grading)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Calculate initial score based on multiple choice questions
  ...
  // Check if quiz has questions before rendering them
