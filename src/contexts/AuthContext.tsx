...
// Define User type to include the name property
type User = {
  id: string;
  name: string;
  email: string;
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Mock authentication functions - in a real app, these would call an API
  const login = async (email: string, password: string, role: string) => {
    try {
      // Simulate API call
      await new Promise(resolve => setTimeout(resolve, 1000));
      
      // Mock success - in a real app, this would validate credentials with a backend
      const mockUser = {
        id: '123',
  ...
  const register = async (name: string, email: string, password: string, role: string) => {
    try {
      ...
      // Mock success - would create an account in a real app
      const mockUser = {
        id: '123',
        name,
        email,
