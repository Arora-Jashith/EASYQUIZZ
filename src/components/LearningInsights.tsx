...
interface LearningInsightsProps {
  ...
  studentName?: string;
  forceRefresh?: number; // Used to force a refresh when a new quiz is completed
}
...
const LearningInsights = ({ 
  ...
  useEffect(() => {
    const fetchInsights = async () => {
      try {
        ...
        // First check if the user has attempted any quizzes
        ...
        if (quizAttempted) {
          // For demonstration, we'll always show insights if there's at least one attempt
    ...
    fetchInsights();
  }, [userId, user?.id, forceRefresh]); // Add forceRefresh as a dependency
  ...
  // Show demo insights for testing even with no attempts
  if (!hasAttemptedQuizzes) {
    // For demo purposes, show insights anyway
    const demoInsights = {
  ...
  // If there are real attempts, show real insights
  return (
    <div className="space-y-6 bg-white text-black p-4 rounded-lg">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <Card className="bg-white border border-gray-200 shadow-sm">
