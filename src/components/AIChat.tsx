...
const AIChat = () => {
  ...
  useEffect(() => {
    const loadChatHistory = async () => {
      ...
      try {
        // Generate a mock user ID until real auth is fully implemented
  ...
  const handleSendMessage = async () => {
    ...
    try {
      // Try to save message, but continue even if it fails
      try {
        await supabase
          .from('chat_messages')
          .insert({
      ...
      // Try to update the message with response, but don't block on failure
      try {
        await supabase
          .from('chat_messages')
